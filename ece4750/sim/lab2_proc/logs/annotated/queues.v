//      // verilator_coverage annotation
 ...
 037004   input  logic enq_val,        // Enqueue data is valid
 000774   output logic enq_rdy,        // Ready for producer to do an enqueue
        
 031563   output logic deq_val,        // Dequeue data is valid
 024528   input  logic deq_rdy,        // Consumer is ready to do a dequeue
        
 026651   output logic write_en,       // Write en signal to wire up to storage element
 026608   output logic bypass_mux_sel, // Used to control bypass mux for bypass queues
 026608   output logic num_free_entries // Either zero or one
        );
        
          // Status register
        
 026608   logic full;
 026651   logic full_next;
        
 127304   always_ff @(posedge clk) begin
 127304     full <= reset ? 1'b0 : full_next;
          end
...
%000000   logic  do_bypass;
 ...
 000198   input  logic                   write_en,
 000198   input  logic                   bypass_mux_sel,
 ...
 000084   input  logic                    clk, reset,
        
 022209   input  logic                    enq_val,        // Enqueue data is valid
%000000   output logic                    enq_rdy,        // Ready for producer to enqueue
        
 015753   output logic                    deq_val,        // Dequeue data is valid
 020493   input  logic                    deq_rdy,        // Consumer is ready to dequeue
        
 013741   output logic                    write_en,       // Wen to wire to regfile
 007402   output logic [c_addr_nbits-1:0] write_addr,     // Waddr to wire to regfile
 007396   output logic [c_addr_nbits-1:0] read_addr,      // Raddr to wire to regfile
 013716   output logic                    bypass_mux_sel, // Control mux for bypass queues
 013716   output logic [c_addr_nbits:0]   num_free_entries // Num of free entries in queue
        );
        
          // Enqueue and dequeue pointers
        
 007402   logic [c_addr_nbits-1:0] enq_ptr;
 007421   logic [c_addr_nbits-1:0] enq_ptr_next;
 ...
 007396   logic [c_addr_nbits-1:0] deq_ptr;
 007402   logic [c_addr_nbits-1:0] deq_ptr_next;
...
%000000   logic  do_pipe;
 ...
 007396   logic [c_addr_nbits-1:0] deq_ptr_plus1;
          assign deq_ptr_plus1 = deq_ptr + 1'b1;
        
          /* verilator lint_off WIDTH */
        
 007396   logic [c_addr_nbits-1:0] deq_ptr_inc;
          assign deq_ptr_inc = (deq_ptr_plus1 == p_num_msgs) ? {c_addr_nbits{1'b0}} : deq_ptr_plus1;
        
 007402   logic [c_addr_nbits-1:0] enq_ptr_plus1;
          assign enq_ptr_plus1 = enq_ptr + 1'b1;
        
 007402   logic [c_addr_nbits-1:0] enq_ptr_inc;
 ...
 013716   input  logic                    bypass_mux_sel,
...
%000000   logic [p_msg_nbits-1:0] read_data;
 ...
 000240   input  logic                   enq_val,
 000774   output logic                   enq_rdy,
 000664   input  logic [p_msg_nbits-1:0] enq_msg,
        
 000240   output logic                   deq_val,
 006770   input  logic                   deq_rdy,
 000554   output logic [p_msg_nbits-1:0] deq_msg,
        
 000198   output logic [c_addr_nbits:0]  num_free_entries
        );
